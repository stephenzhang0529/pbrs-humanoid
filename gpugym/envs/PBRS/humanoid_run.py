...
class HumanoidRun(LeggedRobot):
    ...
    def _custom_init(self, cfg):
        ...
        self.eps = 0.2
        self.phase_freq = 2.0# 提高步态频率 1.0到2.0
    ...
    def _custom_reset(self, env_ids):
        ...
        if self.cfg.commands.resampling_time == -1:
            ...
            self.commands[env_ids, 0:2] = torch.rand(
                (len(env_ids), 2), device=self.device) * 2.5  # 增大线速度范围
            self.commands[env_ids, 2] = (torch.rand((len(env_ids),), device=self.device) - 0.5) * 2  # 允许更高的角速度
...
# ########################## REWARDS ######################## #
    ...
    def _reward_tracking_lin_vel(self):
        ...
        # 额外增加高速奖励
        speed_bonus = torch.norm(self.base_lin_vel[:, :2], dim=1) / 3.0  # 3m/s 速度时奖励达到最大
        return torch.exp(-error/self.cfg.rewards.tracking_sigma)+ speed_bonus
    ...
    def _reward_orientation(self):
        ...
        return torch.exp(-error/self.cfg.rewards.tracking_sigma*2)# 放宽对身体直立性的要求

    def _reward_dof_vel(self):
        # Reward zero dof velocities
        dof_vel_scaled = self.dof_vel/(self.cfg.normalization.obs_scales.dof_vel * 2.0)  # 允许更高关节速度
...
# ##################### HELPER FUNCTIONS ################################## #
    ...
    def smooth_sqr_wave(self, phase):
        ...
        return torch.sin(p) / (1.5 * torch.sqrt(torch.sin(p) ** 2. + self.eps ** 2.)) + 0.5  # 适应更快步态
